...
entry fun insecure_play(r: &Random, payment: Coin<HC>, ...) {
  ...
...
module games::dice {
  ...
  public fun play_dice(guess: u8, fee: Coin<HC>, r: &Random, ctx: &mut TxContext): Ticket {
    // Pay for the turn
...
For example, consider the `entry play_dice(guess: u8, fee: Coin<HC>, r: &Random, ctx: &mut TxContext): Ticket { … }` function defined earlier, the attacker can publish the function
