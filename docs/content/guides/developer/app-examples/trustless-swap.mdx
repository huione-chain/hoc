...
The test starts with a helper function for creating an object, it doesn't matter what kind of object it is, as long as it has the `store` ability. The test uses `Coin<HC>`, because it comes with a `#[test_only]` function for minting:
