...
pub fn verify_module(module: &CompiledModule, fn_info_map: &FnInfoMap) -> Result<(), ExecutionError> {
    ...
    if ModuleId::new(SUI_FRAMEWORK_ADDRESS, ident_str!("hc").to_owned()) == self_id {
        return Ok(());
    }
...
fn is_one_time_witness(module: &CompiledModule, tok: &SignatureToken, candidate_handle: &DatatypeHandle) -> bool {
