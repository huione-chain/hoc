...
fn get_registry() -> Result<Registry> {
    let config = TracerConfig::default().record_samples_for_structs(true).record_samples_for_newtype_structs(true);
    ...
    tracer.trace_value(&mut samples, &Identifier::new("foo").unwrap()).unwrap();
    ...
    let kp1: SuiKeyPair = SuiKeyPair::Ed25519(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp2: SuiKeyPair = SuiKeyPair::Secp256k1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    let kp3: SuiKeyPair = SuiKeyPair::Secp256r1(get_key_pair_from_rng(&mut StdRng::from_seed([0; 32])).1);
    ...
    let multisig_pk =
        MultiSigPublicKey::new(vec![kp1.public(), kp2.public(), kp3.public(), pk_zklogin], vec![1, 1, 1, 1], 2).unwrap();

    let msg = IntentMessage::new(Intent::sui_transaction(), PersonalMessage { message: "Message".as_bytes().to_vec() });
    ...
    let multi_sig =
        MultiSig::combine(vec![sig1.clone(), sig2.clone(), sig3.clone(), sig4.clone()], multisig_pk).unwrap();
    tracer.trace_value(&mut samples, &multi_sig).unwrap();

    let generic_sig_multi = GenericSignature::MultiSig(multi_sig);
    tracer.trace_value(&mut samples, &generic_sig_multi).unwrap();
    ...
    tracer.trace_type::<FullCheckpointContents>(&samples).unwrap();
    ...
    tracer.trace_type::<CertifiedCheckpointSummary>(&samples).unwrap();
