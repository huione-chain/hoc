...
pub async fn send_and_confirm_transaction(
    ...
    let certificate = CertifiedTransaction::new(transaction.into_message(), vec![vote.clone()], &committee)
        .unwrap()
    ...
    let state_acc = StateAccumulator::new_for_tests(authority.get_accumulator_store().clone(), &epoch_store);
    ...
    let effects_acc = state_acc.accumulate_effects(vec![result.inner().data().clone()], epoch_store.protocol_config());
    ...
    R: rand::CryptoRng + rand::RngCore,
{
    ...
    let network_config = sui_swarm_config::network_config_builder::ConfigBuilder::new(&dir).rng(rng).build();
    let genesis = network_config.genesis;
    let authority_key = network_config.validator_configs[0].protocol_key_pair().copy();
...
pub fn create_fake_cert_and_effect_digest<'a>(
    signers: impl Iterator<Item = (&'a AuthorityName, &'a (dyn Signer<AuthoritySignature> + Send + Sync))>,
    committee: &Committee,
...
) -> (Genesis, Vec<(AuthorityPublicKeyBytes, AuthorityKeyPair)>, ObjectID) {
...
) -> (AuthorityAggregator<LocalAuthorityClient>, Vec<Arc<AuthorityState>>, Genesis, ObjectID) {
    let (genesis, key_pairs, framework) = init_genesis(committee_size, genesis_objects).await;
    let authorities = join_all(
        key_pairs
            .iter()
            .map(|(_, key_pair)| TestAuthorityBuilder::new().with_genesis_and_keypair(&genesis, key_pair).build()),
    )
    .await;
...
pub fn make_dummy_tx(receiver: SuiAddress, sender: SuiAddress, sender_sec: &AccountKeyPair) -> Transaction {
