...
impl GovernanceReadApi {
    ...
    async fn get_staked_sui(&self, owner: SuiAddress) -> Result<Vec<StakedSui>, Error> {
        ...
        let result = spawn_monitored_task!(async move { state.get_staked_sui(owner).await }).await??;
    ...
    async fn get_stakes_by_ids(&self, staked_sui_ids: Vec<ObjectID>) -> Result<Vec<DelegatedStake>, Error> {
        let state = self.state.clone();
        let stakes_read = spawn_monitored_task!(async move {
            staked_sui_ids.iter().map(|id| state.get_object_read(id)).collect::<Result<Vec<_>, _>>()
        })
        .await??;
        ...
        for stake in stakes_read.into_iter() {
            match stake {
                ...
                ObjectRead::Deleted(oref) => {
                    match self.state.find_object_lt_or_eq_version(&oref.0, &oref.1.one_before().unwrap()).await? {
                        Some(o) => stakes.push((StakedSui::try_from(&o)?, false)),
                        None => Err(SuiRpcInputError::UserInputError(UserInputError::ObjectNotFound {
                            object_id: oref.0,
                            version: None,
                        }))?,
                    }
                }
                ObjectRead::NotExists(id) => Err(SuiRpcInputError::UserInputError(UserInputError::ObjectNotFound {
                    object_id: id,
                    version: None,
                }))?,
            }
    ...
    async fn get_delegated_stakes(&self, stakes: Vec<(StakedSui, bool)>) -> Result<Vec<DelegatedStake>, Error> {
        let pools = stakes.into_iter().fold(BTreeMap::<_, Vec<_>>::new(), |mut pools, (stake, exists)| {
            pools.entry(stake.pool_id()).or_default().push((stake, exists));
            pools
        });

        let system_state = self.get_system_state()?;
        let system_state_summary: SuiSystemStateSummary = system_state.clone().into_sui_system_state_summary();
        ...
        for (pool_id, stakes) in pools {
            ...
            let rate_table = rates.get(&pool_id).ok_or_else(|| {
                SuiRpcInputError::GenericNotFound("Cannot find rates for staking pool {pool_id}".to_string())
            })?;
...
impl GovernanceReadApiServer for GovernanceReadApi {
    ...
    async fn get_committee_info(&self, epoch: Option<BigInt<u64>>) -> RpcResult<SuiCommittee> {
        with_tracing!(async move {
            self.state.get_or_latest_committee(epoch).map(|committee| committee.into()).map_err(Error::from)
        })
    }
    ...
    async fn get_latest_sui_system_state(&self) -> RpcResult<SuiSystemStateSummary> {
        with_tracing!(
            async move { Ok(self.state.get_system_state().map_err(Error::from)?.into_sui_system_state_summary()) }
        )
    }
    ...
    async fn get_validators_apy(&self) -> RpcResult<ValidatorApys> {
        ...
        Ok(ValidatorApys { apys, epoch: system_state_summary.epoch })
    }
}
...
pub fn calculate_apys(
    ...
    for rates in exchange_rate_table.into_iter().filter(|r| r.active) {
        ...
        let exchange_rates =
            rates.rates.into_iter().filter_map(
                |(epoch, rate)| {
        ...
        let average_apy = if exchange_rates.clone().count() >= 2 {
            ...
            let apys =
                er_e.zip(er_e_1).map(calculate_apy).filter(|apy| *apy > 0.0 && *apy < 0.1).take(30).collect::<Vec<_>>();
        ...
        apys.push(ValidatorApy { address: rates.address, apy: average_apy });
    }
    apys
}
...
fn test_apys_calculation_filter_outliers() {
    ...
    let rates: BTreeMap<String, Vec<(u64, PoolTokenExchangeRate)>> = serde_json::from_reader(file).unwrap();
    ...
    let exchange_rates = rates
        ...
        .map(|(validator, rates)| {
            ...
            ValidatorExchangeRates { address, pool_id: ObjectID::random(), active: true, rates }
        })
        .collect();
...
fn backfill_rates(rates: Vec<(EpochId, PoolTokenExchangeRate)>) -> Vec<(EpochId, PoolTokenExchangeRate)> {
    if rates.is_empty() {
        return rates;
    }
...
mod tests {
    ...
    fn test_backfill_rates_no_gaps() {
        ...
        let expected: Vec<(u64, PoolTokenExchangeRate)> = vec![(3, rate3.clone()), (2, rate2), (1, rate1)];
    ...
    fn test_backfill_rates_with_gaps() {
        ...
        let expected = vec![(5, rate5.clone()), (4, rate3.clone()), (3, rate3.clone()), (2, rate1.clone()), (1, rate1)];
        assert_eq!(backfill_rates(rates), expected);
    }
}
