...
impl TestCaseImpl for CoinIndexTest {
    ...
    async fn run(&self, ctx: &mut TestContext) -> Result<(), anyhow::Error> {
        ...
        let Balance { coin_object_count: mut old_coin_object_count, total_balance: mut old_total_balance, .. } =
        ...
        let response = client
            ...
            .execute_transaction_block(
                txn,
                SuiTransactionBlockResponseOptions::new().with_effects().with_balance_changes(),
                Some(ExecuteTransactionRequestType::WaitForLocalExecution),
            )
            .await?;

        let balance_change = response.balance_changes.unwrap();
        let owner_balance = balance_change.iter().find(|b| b.owner == Owner::AddressOwner(account)).unwrap();
        let recipient_balance = balance_change.iter().find(|b| b.owner != Owner::AddressOwner(account)).unwrap();
        let Balance { coin_object_count, total_balance, coin_type, .. } =
            client.coin_read_api().get_balance(account, None).await?;
        assert_eq!(coin_type, GAS::type_().to_string());

        assert_eq!(coin_object_count, old_coin_object_count);
        assert_eq!(total_balance, (old_total_balance as i128 + owner_balance.amount) as u128);
        old_coin_object_count = coin_object_count;
        old_total_balance = total_balance;

        let Balance { coin_object_count, total_balance, .. } =
            client.coin_read_api().get_balance(recipient_balance.owner.get_owner_address().unwrap(), None).await?;
        assert_eq!(coin_object_count, 1);
        assert!(recipient_balance.amount > 0);
        assert_eq!(total_balance, recipient_balance.amount as u128);

        // 2. Test Staking
        let validator_addr = ctx.get_latest_sui_system_state().await.active_validators.first().unwrap().sui_address;
        ...
        let Balance { coin_object_count, total_balance, .. } = client.coin_read_api().get_balance(account, None).await?;
        ...
        let Balance { total_balance, .. } = client.coin_read_api().get_balance(account, None).await?;
        old_total_balance = total_balance;

        info!("token package published, package: {:?}, cap: {:?}", package, cap);
        let sui_type_str = "0x2::hc::HC";
        ...
        let txn = client
            .transaction_builder()
            .move_call(account, package.0, "managed", "mint", vec![], args, None, rgp * 2_000_000, None)
            .await
            .unwrap();
        let response = ctx.sign_and_execute(txn, "mint managed coin to self").await;

        let balance_changes = &response.balance_changes.unwrap();
        let sui_balance_change = balance_changes.iter().find(|b| b.coin_type.to_string().contains("SUI")).unwrap();
        let managed_balance_change =
            balance_changes.iter().find(|b| b.coin_type.to_string().contains("MANAGED")).unwrap();
        ...
        let Balance {
            ...
            ..
        } = client.coin_read_api().get_balance(account, Some(coin_type_str.clone())).await?;
        ...
        let managed_balance = client.coin_read_api().get_balance(account, Some(coin_type_str.clone())).await.unwrap();
        let managed_coins =
            client.coin_read_api().get_coins(account, Some(coin_type_str.clone()), None, None).await.unwrap().data;
        ...
        let managed_coin_id = managed_coins.iter().find(|c| c.balance == 10).unwrap().coin_object_id;
        let managed_coin_id_10k = managed_coins.iter().find(|c| c.balance == 10000).unwrap().coin_object_id;
        let _ = add_to_envelope(ctx, package.0, envelope.0, managed_coin_id).await;

        let managed_balance = client.coin_read_api().get_balance(account, Some(coin_type_str.clone())).await.unwrap();
        assert_eq!(managed_balance.total_balance, managed_old_total_balance - 10);
        assert_eq!(managed_balance.coin_object_count, managed_old_total_count - 1);
        let managed_old_total_balance = managed_balance.total_balance;
        let managed_old_total_count = managed_balance.coin_object_count;
        ...
        let txn = client
            .transaction_builder()
            .move_call(account, package.0, "managed", "take_from_envelope", vec![], args, None, rgp * 2_000_000, None)
            .await
            .unwrap();
        let response = ctx.sign_and_execute(txn, "take back managed coin from envelope").await;
        assert!(response.status_ok().unwrap());
        let managed_balance = client.coin_read_api().get_balance(account, Some(coin_type_str.clone())).await.unwrap();
        assert_eq!(managed_balance.total_balance, managed_old_total_balance + 10);
        assert_eq!(managed_balance.coin_object_count, managed_old_total_count + 1);
        ...
        let txn = client
            ...
            .move_call(
                ...
                vec![],
                vec![SuiJsonValue::from_object_id(cap.0), SuiJsonValue::from_object_id(envelope.0)],
                None,
                rgp * 2_000_000,
                None,
            )
            .await
            .unwrap();
        let response = ctx.sign_and_execute(txn, "take back managed coin from envelope and burn").await;
        assert!(response.status_ok().unwrap());
        let managed_balance = client.coin_read_api().get_balance(account, Some(coin_type_str.clone())).await.unwrap();
        ...
        let txn = client
            ...
            .move_call(
                ...
                "burn",
                vec![],
                vec![SuiJsonValue::from_object_id(cap.0), SuiJsonValue::from_object_id(managed_coin_id_10k)],
                None,
                rgp * 2_000_000,
                None,
            )
        ...
        let sui_coins =
            client.coin_read_api().get_coins(account, Some(sui_type_str.into()), None, None).await.unwrap().data;

        assert_eq!(sui_coins, client.coin_read_api().get_coins(account, None, None, None).await.unwrap().data,);
        assert_eq!(
            // this is only SUI coins at the moment
            sui_coins,
            client.coin_read_api().get_all_coins(account, None, None).await.unwrap().data,
        );

        let sui_balance = client.coin_read_api().get_balance(account, None).await.unwrap();
        assert_eq!(sui_balance.total_balance, sui_coins.iter().map(|c| c.balance as u128).sum::<u128>());
        ...
        assert_eq!(
            sui_coins,
            client.coin_read_api().get_coins(account, None, None, Some(sui_coins.len() + 1)).await.unwrap().data,
        );
        ...
        let mut cursor = None;
        loop {
            let page = client.coin_read_api().get_all_coins(account, cursor, None).await.unwrap();
        ...
        assert_ne!(managed_coins_2_11.data.first().unwrap().coin_object_id, first_managed_coin);
        ...
        assert_eq!(managed_coins_12_39.data.last().unwrap().coin_object_id, last_managed_coin);
        assert!(!managed_coins_12_39.data.iter().any(|coin| coin.coin_object_id == removed_coin_id));
...
async fn publish_managed_coin_package(
    ...
    let pkg = changes.iter().find(|change| matches!(change, ObjectChange::Published { .. })).unwrap().object_ref();
...
async fn add_to_envelope(
    ...
    let txn = client
        ...
        .move_call(
            ...
            "add_to_envelope",
            vec![],
            vec![SuiJsonValue::from_object_id(envelope), SuiJsonValue::from_object_id(coin)],
            None,
            rgp * 2_000_000,
            None,
        )
        .await
        .unwrap();
    let response = ctx.sign_and_execute(txn, "add managed coin to envelope").await;
    assert!(response.status_ok().unwrap());
    response
}
