...
async fn test_get_objects() -> Result<(), anyhow::Error> {
    ...
    let object_digests = objects.data.iter().map(|o| o.object().unwrap().object_id).collect();
...
async fn test_get_package_with_display_should_not_fail() -> Result<(), anyhow::Error> {
    ...
    let response = http_client
        .get_object(ObjectID::from(SUI_FRAMEWORK_ADDRESS), Some(SuiObjectDataOptions::new().with_display()))
        .await;
    assert!(response.is_ok());
    let response: SuiObjectResponse = response?;
    assert!(response.into_object().unwrap().display.unwrap().data.is_none());
    Ok(())
}
...
async fn test_public_transfer_object() -> Result<(), anyhow::Error> {
    ...
    let tx_response: SuiTransactionBlockResponse = http_client
        .execute_transaction_block(
            tx_bytes1,
            signatures,
            Some(SuiTransactionBlockResponseOptions::new().with_effects().with_object_changes()),
...
fn assert_same_object_changes_ignoring_version_and_digest(expected: Vec<ObjectChange>, actual: Vec<ObjectChange>) {
    fn collect_changes_mask_version_and_digest(changes: Vec<ObjectChange>) -> BTreeMap<ObjectID, ObjectChange> {
        changes
            .into_iter()
            .map(|mut change| {
...
async fn test_publish() -> Result<(), anyhow::Error> {
    ...
    let compiled_package = BuildConfig::new_for_testing().build(Path::new("../../examples/move/basics"))?;
...
async fn test_get_object_info() -> Result<(), anyhow::Error> {
    ...
    for obj in objects {
        ...
        let result = http_client.get_object(oref.object_id, Some(SuiObjectDataOptions::new().with_owner())).await?;
        assert!(
...
async fn test_get_object_data_with_content() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(
            address,
            Some(SuiObjectResponseQuery::new_with_options(SuiObjectDataOptions::new().with_content().with_owner())),
            None,
            None,
        )
        .await?
    ...
    for obj in objects {
        ...
        let result = http_client
            .get_object(oref.object_id, Some(SuiObjectDataOptions::new().with_content().with_owner()))
            .await?;
        assert!(
            matches!(result, SuiObjectResponse { data: Some(object), .. } if oref.object_id == object.object_id && object.owner.clone().unwrap().get_owner_address()? == address)
        );
...
async fn test_get_coins() -> Result<(), anyhow::Error> {
    ...
    let result: CoinPage = http_client.get_coins(address, Some("0x2::hc::HC".into()), None, None).await?;
    assert_eq!(5, result.data.len());
    assert!(!result.has_next_page);

    // Test paging
    let result: CoinPage = http_client.get_coins(address, Some("0x2::hc::HC".into()), None, Some(3)).await?;
    assert_eq!(3, result.data.len());
    assert!(result.has_next_page);

    let result: CoinPage =
        http_client.get_coins(address, Some("0x2::hc::HC".into()), result.next_cursor, Some(3)).await?;
    assert_eq!(2, result.data.len(), "{:?}", result);
    assert!(!result.has_next_page);

    let result: CoinPage = http_client.get_coins(address, Some("0x2::hc::HC".into()), result.next_cursor, None).await?;
...
async fn test_get_balance() -> Result<(), anyhow::Error> {
    ...
    assert_eq!("0x2::hc::HC", result.coin_type);
...
async fn test_get_metadata() -> Result<(), anyhow::Error> {
    ...
    let tx_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(SuiTransactionBlockResponseOptions::new().with_object_changes().with_events()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;
    ...
    let package_id = object_changes
        .iter()
        .find_map(|e| if let ObjectChange::Published { package_id, .. } = e { Some(package_id) } else { None })
        .unwrap();

    let result: SuiCoinMetadata =
        http_client.get_coin_metadata(format!("{package_id}::trusted_coin::TRUSTED_COIN")).await?.unwrap();
...
async fn test_get_total_supply() -> Result<(), anyhow::Error> {
    ...
    let treasury_cap = object_changes
        .iter()
        .find_map(|e| {
            if let ObjectChange::Created { object_id, object_type, .. } = e {
...
async fn test_unstaking() -> Result<(), anyhow::Error> {
    let cluster = TestClusterBuilder::new().with_epoch_duration_ms(10000).build().await;
...
async fn test_staking_multiple_coins() -> Result<(), anyhow::Error> {
    ...
    let transaction_bytes: TransactionBlockBytes = http_client
        .request_add_stake(
            address,
            vec![coins.data[0].coin_object_id, coins.data[1].coin_object_id, coins.data[2].coin_object_id],
            Some(1000000000.into()),
            validator,
            None,
            100_000_000.into(),
    ...
    let dryrun_response = http_client.dry_run_transaction_block(tx_bytes.clone()).await?;

    let executed_response = http_client
        .execute_transaction_block(
            tx_bytes,
            signatures,
            Some(SuiTransactionBlockResponseOptions::new().with_balance_changes().with_input()),
            Some(ExecuteTransactionRequestType::WaitForLocalExecution),
        )
        .await?;

    // Check coin balance changes on dry run
    assert_eq!(dryrun_response.balance_changes, executed_response.balance_changes.unwrap());
