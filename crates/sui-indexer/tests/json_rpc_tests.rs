...
use sui_json_rpc_types::{CoinPage, EventFilter, SuiObjectDataOptions, SuiObjectResponse, SuiObjectResponseQuery};
...
async fn test_get_owned_objects() -> Result<(), anyhow::Error> {
    ...
    let objects = http_client
        .get_owned_objects(address, Some(SuiObjectResponseQuery::new_with_options(data_option.clone())), None, None)
        .await?
        .data;
    ...
    for obj in &objects {
        ...
        let result = http_client.get_object(oref.object_id, Some(data_option.clone())).await?;
        assert!(
    ...
    let object_ids: Vec<_> = objects.iter().map(|o| o.object().unwrap().object_id).collect();

    let object_resp = http_client.multi_get_objects(object_ids.clone(), None).await?;
    let fullnode_object_resp = cluster.fullnode_handle.rpc_client.multi_get_objects(object_ids, None).await?;
...
async fn test_event_type_filter() {
    ...
    let result = client
        .query_events(
            EventFilter::MoveEventType("0x3::validator_set::ValidatorEpochInfoEventV2".parse().unwrap()),
            None,
            None,
            None,
        )
    ...
    let result = client
        .query_events(
            EventFilter::MoveEventType("0x0003::validator_set::ValidatorEpochInfoEventV2".parse().unwrap()),
            None,
            None,
            None,
        )
    ...
    let result = client
        .query_events(
            EventFilter::MoveEventType("0x1::validator_set::ValidatorEpochInfoEventV2".parse().unwrap()),
            None,
            None,
            None,
        )
