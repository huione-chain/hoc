...
use sui_types::{
    ...
    quorum_driver_types::{ExecuteTransactionRequestType, ExecuteTransactionRequestV3, QuorumDriverResponse},
    storage::ObjectStore,
    transaction::{
        CallArg,
        GasData,
...
async fn test_full_node_follows_txes() -> Result<(), anyhow::Error> {
    ...
    fullnode.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await;
...
async fn test_full_node_shared_objects() -> Result<(), anyhow::Error> {
    ...
    let response = increment_counter(context, sender, None, package_ref.0, counter_ref.0, counter_ref.1).await;
...
async fn test_sponsored_transaction() -> Result<(), anyhow::Error> {
    ...
    let (sent_coin, sender_, receiver, _, object_ref) = transfer_coin(&test_cluster.wallet).await.unwrap();
    ...
    let tx = to_sender_signed_transaction_with_multi_signers(tx_data, vec![
        test_cluster.wallet.config.keystore.get_key(&sender).unwrap(),
        test_cluster.wallet.config.keystore.get_key(&sponsor).unwrap(),
    ]);

    test_cluster.execute_transaction(tx).await;

    assert_eq!(sponsor, test_cluster.wallet.get_object_owner(&sent_coin).await.unwrap(),);
    Ok(())
}
...
async fn test_full_node_move_function_index() -> Result<(), anyhow::Error> {
    ...
    let txes = node
        .state()
        .get_transactions_for_tests(
            Some(TransactionFilter::MoveFunction { package: package_ref.0, module: None, function: None }),
            None,
            None,
            false,
        )
...
async fn test_full_node_indexes() -> Result<(), anyhow::Error> {
    telemetry_subscribers::init_for_testing();
    let mut test_cluster = TestClusterBuilder::new().enable_fullnode_events().build().await;
    let node = &test_cluster.fullnode_handle.sui_node;
    let context = &mut test_cluster.wallet;
    ...
    let txes = node
        .state()
        .get_transactions_for_tests(Some(TransactionFilter::InputObject(transferred_object)), None, None, false)
        .await?;
    ...
    let txes = node
        .state()
        .get_transactions_for_tests(Some(TransactionFilter::ChangedObject(transferred_object)), None, None, false)
        .await?;
    assert_eq!(txes.len(), 2);
    assert_eq!(txes[1], digest);

    let txes =
        node.state().get_transactions_for_tests(Some(TransactionFilter::FromAddress(sender)), None, None, false).await?;
    assert_eq!(txes.len(), 1);
    assert_eq!(txes[0], digest);

    let txes =
        node.state().get_transactions_for_tests(Some(TransactionFilter::ToAddress(receiver)), None, None, false).await?;
    assert_eq!(txes.len(), 2);
    assert_eq!(txes[1], digest);
    ...
    let txes =
        node.state().get_transactions_for_tests(Some(TransactionFilter::ToAddress(sender)), None, None, false).await?;
    assert_eq!(txes.len(), 2);
    assert_eq!(txes[1], digest);
    ...
    let txes = node
        .state()
        .get_transactions_for_tests(Some(TransactionFilter::FromAddress(receiver)), None, None, false)
        .await?;
    ...
    let sender_balance_change = BalanceChange {
        ...
        owner: sender,
        coin_type: parse_struct_tag("0x2::hc::HC").unwrap(),
...
async fn test_full_node_cold_sync() -> Result<(), anyhow::Error> {
    ...
    let info =
        fullnode.state().handle_transaction_info_request(TransactionInfoRequest { transaction_digest: digest }).await?;
...
async fn test_full_node_event_read_api_ok() {
    let mut test_cluster =
        TestClusterBuilder::new().with_fullnode_rpc_port(50000).enable_fullnode_events().build().await;
    ...
    let events: Vec<SuiEvent> = jsonrpc_client.request("sui_getEvents", params).await.unwrap();
...
async fn test_full_node_event_query_by_module_ok() {
    ...
    let params =
        rpc_params![EventFilter::MoveEventModule { package: package_id, module: ident_str!("testnet_nft").into() }];
    let page: EventPage = jsonrpc_client.request("suix_queryEvents", params).await.unwrap();
...
async fn test_full_node_transaction_orchestrator_basic() -> Result<(), anyhow::Error> {
    ...
    let kv_store = Arc::new(TransactionKeyValueStore::new("rocksdb", metrics, fullnode.state()));

    let context = &mut test_cluster.wallet;
    let transaction_orchestrator = fullnode.with(|node| {
        node.transaction_orchestrator().expect("Fullnode should have transaction orchestrator toggled on.")
    });
    ...
    let (tx, QuorumDriverResponse { effects_cert: certified_txn_effects, events: txn_events, .. }) =
        rx.recv().await.unwrap().unwrap();
    let (response, is_executed_locally) = res;
    assert_eq!(*tx.digest(), digest);
    assert_eq!(response.effects.effects.digest(), *certified_txn_effects.digest());
    assert!(is_executed_locally);
    assert_eq!(response.events.unwrap_or_default().digest(), txn_events.unwrap_or_default().digest());
    // verify that the node has sequenced and executed the txn
    fullnode.state().get_executed_transaction_and_effects(digest, kv_store.clone()).await.unwrap_or_else(|e| {
        panic!("Fullnode does not know about the txn {:?} that was executed with WaitForLocalExecution: {:?}", digest, e)
    });
    ...
    assert_eq!(*tx.digest(), digest);
    assert_eq!(response.effects.effects.digest(), *certified_txn_effects.digest());
    assert_eq!(txn_events.unwrap_or_default().digest(), response.events.unwrap_or_default().digest());
    assert!(!is_executed_locally);
    fullnode.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await;
    fullnode.state().get_executed_transaction_and_effects(digest, kv_store).await.unwrap_or_else(|e| {
        panic!("Fullnode does not know about the txn {:?} that was executed with WaitForEffectsCert: {:?}", digest, e)
    });
...
async fn test_validator_node_has_no_transaction_orchestrator() {
    ...
    node_handle.with(|node| {
        ...
        assert!(node.subscribe_to_transaction_orchestrator_effects().is_err());
    });
}
...
async fn test_execute_tx_with_serialized_signature() -> Result<(), anyhow::Error> {
    let mut test_cluster = TestClusterBuilder::new().build().await;
    let context = &mut test_cluster.wallet;
    context.config.keystore.add_key(None, SuiKeyPair::Secp256k1(get_key_pair().1))?;
...
async fn test_get_objects_read() -> Result<(), anyhow::Error> {
    ...
    let gas_ref = test_cluster.wallet.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap();
    let nft_transfer_tx = test_cluster
        .wallet
        .sign_transaction(&TestTransactionBuilder::new(sender, gas_ref, rgp).transfer(object_ref_v1, recipient).build());
    ...
    let read_ref_v3 = match node.state().get_past_object_read(&object_id, object_ref_v3.1)? {
        PastObjectRead::ObjectDeleted(obj_ref) => obj_ref,
        other => anyhow::bail!("Expect object {object_id:?} deleted but got {other:?}."),
    };
    assert_eq!(object_ref_v3, read_ref_v3);

    let (read_ref_v2, read_obj_v2, _) = get_past_obj_read_from_node(node, object_id, object_ref_v2.1).await?;
    ...
    let (read_ref_v1, read_obj_v1, _) = get_past_obj_read_from_node(node, object_id, object_ref_v1.1).await?;
    ...
    match node.state().get_past_object_read(&object_id, too_high_version)? {
        PastObjectRead::VersionTooHigh { object_id: obj_id, asked_version, latest_version } => {
            assert_eq!(obj_id, object_id);
            assert_eq!(asked_version, too_high_version);
            assert_eq!(latest_version, object_ref_v3.1);
        }
        other => anyhow::bail!("Expect SequenceNumberTooHigh for object {object_id:?} but got {other:?}."),
    };
...
async fn test_full_node_bootstrap_from_snapshot() -> Result<(), anyhow::Error> {
    ...
    let checkpoint_path = test_cluster.fullnode_handle.sui_node.with(|node| node.db_checkpoint_path());
    let config = test_cluster.fullnode_config_builder().build(&mut OsRng, test_cluster.swarm.config());
    ...
    let node = test_cluster.start_fullnode_from_config(config).await.sui_node;

    node.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest]).await;
    ...
    let (_transferred_object, _, _, digest_after_restore, ..) = transfer_coin(&test_cluster.wallet).await?;
    node.state().get_transaction_cache_reader().notify_read_executed_effects(&[digest_after_restore]).await;
    Ok(())
}
...
async fn test_pass_back_no_object() -> Result<(), anyhow::Error> {
    ...
    let gas_obj = context.get_one_gas_object_owned_by_address(sender).await.unwrap().unwrap();
    ...
    .unwrap();
    let tx = to_sender_signed_transaction(tx_data, context.config.keystore.get_key(&sender).unwrap());
    ...
    let (_tx, QuorumDriverResponse { effects_cert: _certified_txn_effects, events: _txn_events, .. }) =
        rx.recv().await.unwrap().unwrap();
    Ok(())
}
        ...
        validator
            ...
            .with_async(|node| async {
                ...
                state
                    ...
                    .prune_objects_and_compact_for_testing(state.get_checkpoint_store(), state.rpc_index.as_deref())
                    .await;
                // Make sure the old version of the object is already pruned.
                assert!(state.database_for_testing().get_object_by_key(&gas_object.0, gas_object.1).is_none());
                let epoch_store = state.epoch_store_for_testing();
                assert_eq!(
                    state
                        .handle_transaction(&epoch_store, epoch_store.verify_transaction(tx.clone()).unwrap())
                        .await
                        .unwrap_err(),
...
async fn transfer_coin(
...
) -> Result<(ObjectID, SuiAddress, SuiAddress, TransactionDigest, ObjectRef), anyhow::Error> {
    ...
    let txn = context.sign_transaction(
        &TestTransactionBuilder::new(sender, gas_object, gas_price).transfer(object_to_send, receiver).build(),
    );
...
async fn test_full_node_run_with_range_checkpoint() -> Result<(), anyhow::Error> {
    ...
    assert!(test_cluster.fullnode_handle.sui_node.with(|node| {
        node.state().get_checkpoint_store().get_highest_executed_checkpoint_seq_number().unwrap()
    ...
    assert!(test_cluster.fullnode_handle.sui_node.with(|node| node.transaction_orchestrator()).is_none());
    Ok(())
}
...
async fn test_full_node_run_with_range_epoch() -> Result<(), anyhow::Error> {
    ...
    // epoch + 1
    assert!(test_cluster.fullnode_handle.sui_node.with(|node| node.current_epoch_for_testing() == stop_after_epoch + 1));
