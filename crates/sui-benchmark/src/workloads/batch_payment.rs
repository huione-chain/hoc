...
use crate::{
    ...
    workloads::{
        payload::Payload,
        workload::{ExpectedFailureType, Workload, WorkloadBuilder, ESTIMATED_COMPUTATION_COST, STORAGE_COST_PER_COIN},
        Gas,
        GasCoinConfig,
        WorkloadBuilderInfo,
        WorkloadParams,
...
impl Payload for BatchPaymentTestPayload {
    ...
    fn make_transaction(&mut self) -> Transaction {
        ...
        let amount = if self.num_payments == 0 { PRIMARY_COIN_VALUE } else { BATCH_TRANSFER_AMOUNT };
        ...
        let gas_budget = ESTIMATED_COMPUTATION_COST + (STORAGE_COST_PER_COIN * (num_recipients + 1) as u64);
    ...
    pub fn from(
        ...
        if max_ops == 0 || num_workers == 0 {
            ...
            let workload_builder =
                Box::<dyn WorkloadBuilder<dyn Payload>>::from(Box::new(BatchPaymentWorkloadBuilder {
...
impl Workload<dyn Payload> for BatchPaymentWorkload {
    ...
    async fn make_test_payloads(
        ...
        debug!("Making test payloads with {} payload gas...", self.payload_gas.len());
        for gas in self.payload_gas.iter() {
            gas_by_address.entry(gas.1).or_insert_with(|| Vec::with_capacity(1)).push(gas.clone());
        }
        assert_eq!(gas_by_address.len(), self.payload_gas.len(), "Each sender needs some gas");

        let mut payloads = Vec::new();

        debug!("Creating {} senders & {} recipients per sender...", gas_by_address.len(), self.batch_size);
