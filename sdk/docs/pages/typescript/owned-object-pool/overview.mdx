...
default coin selection logic, which uses all the `0x2::coin::Coin<0x2::hc::HC>` objects owned by
...
`0x2::coin::Coin<0x2::hc::HC>` in your wallet for each transaction that you need to execute in
parallel (in our case 10 coins). Each `Coin<HC>` should have enough balance to execute each
